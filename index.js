...
client.on('message-new', async (m) => {
   ...
   switch (prefix) {
       ...
       case 'menu1':
           ...
           break
       case 'vsfdleonardo':
           client.sendMessage(id, vsfdleonardo.vsfdleonardo(id, A187, tanggal, waktu, whatsapp, youtube, instagram, aktif, nomer, ontime), MessageType.text)
           break
       case 'menu2':
      ...
      case 'info':
       ...
       case 'nulis':
           nulis(value)
               .then(data => {
                   client.sendMessage(id, '[ARGUARDE, CASO NÃO OBTENHA TENTE NOVAMENTE...', MessageType.text)
       ...
       case 'twt':
           tweetdl(value)
               .then(data => {
                    ...
                    let hasil = `✄1�71ￄ1�77 Pronto! por favor clique no link abaixo para baixar os resultados!\nClique no link abaixo\n\nSize: ${size}\n\nLink: ${link}`
                    client.sendMessage(id, '[ARGUARDE, CASO NÃO OBTENHA TENTE NOVAMENTE...', MessageType.text)
                    client.sendMessage(id, hasil ,MessageType.text)
               })
               .catch(err => {
 ...
 case 'infogempa':
   ...
   case 'chord':
           chord(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Aqui estão os acordes da música *${value}*\n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
 ...
 case 'covidcountry':
  ...
  case 'infoanime':
           infoanime(value)
               .then(data => {
                   const { result } = data
                   let hasil = `*Informações sobre o anime ${value} :* \n\n _${result}_  `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
...
case 'gay':
           gay()
               .then(data => {
                   ...
                   let hasil = `*${desc} \n\n *Sua porcentagem gay é* _${persen}_  `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
...
case 'filmanime':
           filmanime(value)
               .then(data => {
                   ...
                   let hasil = `*Anime ${value} :* \n\n *título* _${title}_ \n\n *Avaliações* _${rating}_ \n\n *Info* _${sinopsis}_ \n\n *Link Anime* _${video}_  `
  ...
  case 'cektanggal':
   ...
   case 'bitly':
           bitly(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Aqui, link criado amg\n\n${result} `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
    ...
    case 'bucin':
     ...
     case 'cooltext':
          ...
          case 'creator':
            ...
            break
      case 'tts':
            if (args.length === 1) return client.reply(from, 'Enviar pedidos *!tts [id, en, jp, ar] [teks]*, exemplo *!tts en Ola a todos*')
	    ...
	    const ttsJp = require('node-gtts')('ja')
	        ...
	        if (dataBhs == 'id') {
                ...
                client.reply(from, 'Insira os dados do idioma : [id] para indonésio, [en] para ingles, [jp] para japones e [ar] para arabe', id)
            }
            break     
      case 'stickergif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    ...
                    client.reply(from, '[❗] Envie o vídeo com a legenda *!StickerGif* máximo de 10 sec!', id)
                )
            }
            break     
     case 'sticker':
